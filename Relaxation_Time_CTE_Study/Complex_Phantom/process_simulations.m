...
stime=1; %start time (end of compression)
d1=dir(pwd);
tms=cell(3,2);
n=tic;
if exist('./barout.txt','file')~=2
    ...
    if d1(i).isdir
        if exist(['./',d1(i).name,'/run.txt'],'file')==2
            ...
            for j=1:length(d2)
                ...
                tms{1,2}=toc;
                tms{1,1}='ResultsConfigure runtime';
                tic;
                strain_gen_files3(outfile,Nodefile,Outputfile1,stime)
                tms{2,2}=toc;
                tms{2,1}='strain_gen_files2 runtime';
                tic;
                T=curve_fit(Outputfile1);
                tms{3,2}=toc;
                tms{3,1}='curve_fit runtime';
...
save(Outputfile2,'T','tms') %,'i_b_contrast_2','i_b_contrast_1'
