...
-------------------------------------
  ...
  SimList.xml-- XML file containing parameters used by upd_sim_bp.py for template .feb file.

  benign.feb-- template .feb file. File contains the finite element model for the complex breast phantom with a regularly shaped lesion and material characteristics of a benign lesion. Note: the benign model can be changed to the malignent model by adjusting the Elastic (i.e. V-W model) parameters accordingly.
...
If the base name is changed, all instances down the workflow will be changed as well (e.g. Sim**_dat.mat becomes (new basename)**_dat.mat
  SimulationResults(folder)-- simulation results folder.
